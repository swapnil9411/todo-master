...
const app = require("../src/backend/databases/sqlite");
...
describe("Task 1 tests #start_test", function() {
  it("should ensure db file exist", done => {
    f2("./src/backend/databases/sqlite.js").then(res => {
  ...
  it("should ensure db exist #end_test", done => {
    f3("./src/backend/databases/database.sqlite").then(res => {
      expect(res).to.equal(1);done();
      // process.exit();
    });
  });
});
